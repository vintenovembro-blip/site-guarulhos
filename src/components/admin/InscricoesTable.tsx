...
function toCsv(rows: Inscricao[]) {
  const header = ["Nome", "E-mail", "WhatsApp", "Série", "Interesse", "Data"];
  const lines = rows.map((r) =>
    [r.nome, r.email, r.whatsapp, r.serie, r.interesse, new Date(r.created_at).toLocaleString("pt-BR")]
...
export default function InscricoesTable({ inscricoes }: { inscricoes: Inscricao[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return inscricoes.filter((r) =>
      [r.nome, r.email, r.whatsapp, r.serie, r.interesse].some((v) => v.toLowerCase().includes(q))
    );
  }, [inscricoes, query]);
